locals {
  ...
  spoke_id = format("spoke%s", local.spoke_vars.spoke)
...
dependencies {
  paths = [
    "../vpc",
    "../../../../../../networking/landing/useast1/hub-000/vpc-tgw"
  ]
}
...
dependency "tgw" {
  config_path = "../../../../../../networking/landing/useast1/hub-000/vpc-tgw"
  # Configure mock outputs for the `validate` command that are returned when there are no outputs available (e.g the
  # module hasn't been applied yet.
  mock_outputs_allowed_terraform_commands = ["validate"]
  mock_outputs = {
    transit_gateway = {
      ...
      id           = "tgw-abcdefghijk123"
      ram_share_id = ""
      route_ids = [
        "tgw-rtb-abcdefghijk123_0.0.0.0/16",
        "tgw-rtb-abcdefghijk123_100.0.0.0/16",
      ]
      route_table_id = [
        "tgw-attach-abcdefghijk123",
      ]
      route_table_ids = [
        "tgw-rtb-abcdefghijk123_tgw-attach-abcdefghijk123",
      ]
      vpc_attachments = [
        "tgw-attach-abcdefghijk123",
      ]
...
inputs = {
  ...
  shared = {
    ram_share_id       = dependency.tgw.outputs.transit_gateway.ram_share_id
    tgw_route_table_id = dependency.tgw.outputs.transit_gateway.route_table_id
  }
  vpc_attachments = merge({
    (local.spoke_id) = {
      tgw_id               = dependency.tgw.outputs.transit_gateway.id
